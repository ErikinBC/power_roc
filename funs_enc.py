...
class dgp_bin():
    ...
    def __init__(self, mu, p, thresh=None, sens=None, spec=None, alpha=0.05):
        ...
        check_none = (thresh != None) or (sens != None) or (spec != None)
    ...
    def run_bootstrap(self, msr, n_trial, method='basic', n_bs=1000, seed=None, alpha=None):
        lst_method = ['binom', 'basic', 'quantile', 'expanded', 'studentized', 'bca']
        if method != None:
            assert method in lst_method
            di_ci = {method:None}
        else:
            # Calculate all methods except studentized
            di_ci = dict.fromkeys([m for m in lst_method if m != 'studentized'])
        if alpha is None:
            alpha = self.alpha
        ...
        if 'binom' in di_ci:
            np.random.seed(seed)
            theta_binom = cvec(np.random.binomial(n=n, p=theta_hat, size=n_bs) / n)
            power_binom = self.power_binom(theta_binom, h0, n_trial)
            arr = np.quantile(power_binom, np.flip(alpha_vec), axis=0)
            di_ci['binom'] = self.clean_h0(arr, h0)
    ...
    """
    # msr='sens';method='binom';seed=1;n1_trial=100;n0_trial=100
